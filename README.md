# 👋 Hi, I'm Robert Tagliavia

**Full Stack Developer** with 4+ years of experience building scalable web applications and backend systems. I specialize in **Node.js**, **React**, and real-time architectures, and I'm currently expanding into **web3** and **blockchain development**.

---

### 🛠️ Tech Stack

**Backend**  
Node.js • Express • JWT • Webhooks • GraphQL • Socket.io • MySQL • Python

**Frontend**  
React • Next.js • JavaScript

**Integrations & APIs**  
CRM APIs (HubSpot, Monday, Escala, etc.) • Third-party APIs • Asterisk AMI

**Infrastructure & DevOps**  
Linux • Nginx • PM2 • SSH • AWS (EC2, S3) • Git • GitHub

**Currently Learning**  
Solidity • Hardhat • Ethers.js • Web3 dApps

---

### 🚀 What I’m Working On

- Building full‑stack dApps with **React + Solidity** (Hardhat, Sepolia testnet)  
- Crafting interactive dashboards and real‑time messaging platforms  
- Integrating telephony systems via Asterisk AMI and webhooks  

---

### 📫 How to Reach Me

- **Email**: [rtagliavia.pp@gmail.com](mailto:rtagliavia.pp@gmail.com)  
- **LinkedIn**: [linkedin.com/in/rtagliaviaz]([https://www.linkedin.com/in/robert-tagliavia](https://www.linkedin.com/in/rtagliaviaz/)) 
<!--- **Portfolio / Blog**: coming soon!  --->
---
<!---
### 📌 GitHub Stats

![Robert's GitHub stats](https://github-readme-stats.vercel.app/api?username=rtagliaviaz&show_icons=true&theme=radical)

---
--->

⭐️ *I’m open to new opportunities and collaborations in web3, blockchain, and innovative full‑stack projects.*
